...
const Dropdown = ({label, options, name, isRequired = false, value, onChange, errors}) => {
  return (
    <>
      {errors && errors !== "" ? (
        <div>
          <label htmlFor={label} className="block text-sm font-medium mb-2">{capitalizeString(label)} {isRequired ? "*" : ""}</label>
         ...
         <div>
          ...
          {value && <p className="text-sm text-teal-600 mt-2">Looks good!</p>}
        </div>
      )}
